...
public:
	...
	virtual void ExportByte		( const Char* FieldName, Byte Value		) = 0;
	virtual void ExportInteger	( const Char* FieldName, Integer Value	) = 0;
...
public:
	...
	virtual Byte		ImportByte		( const Char* FieldName ) = 0;
	virtual Integer		ImportInteger	( const Char* FieldName ) = 0;
...
#define IMPORT_BYTE(name)		*(Byte*)&name = Im.ImportByte( L#name );
...
#define EXPORT_BYTE(name)		Ex.ExportByte( L#name, (Byte)name );
